...
async function updateUserStats(userId, analysis) {
    ...
    if (!stats) {
        stats = new UserStats_1.default({ userId });
    }
    // ------------------------
    // 🔥 DAILY STREAK LOGIC
    // ------------------------
    ...
    if (!last) {
        ...
        const diff = (new Date(today).getTime() - new Date(last).getTime()) /
            (1000 * 60 * 60 * 24);
    ...
    // ------------------------
    const sessionTitle = analysis.fromDailyQuestion === true
        ? "Daily Question Attempt"
        : "AI Practice Session";
    const sessionScore = analysis.dailyQuestion?.overallScore ??
