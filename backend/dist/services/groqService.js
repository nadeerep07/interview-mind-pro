...
Be concise and practical. Return ONLY the JSON object, no other text.`;
            const response_data = await axios_1.default.post("https://api.groq.com/openai/v1/chat/completions", {
                ...
                headers: {
                    Authorization: `Bearer ${env_1.config.GROQ_API_KEY}`,
...
function getMockAnalysis(question, response) {
    ...
    return {
        score: Math.min(100, Math.max(65, 70 + Math.floor(Math.random() * 20) + (hasExamples ? 5 : 0) + (hasMetrics ? 5 : 0))),
